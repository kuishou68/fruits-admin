	...
	async create() {
		...
		if (goods_id) {
			...
			if (goodsState != '销售中') {
				switch (goodsState) {
					case '待审核':
						response.msg = `当前水果未开售`;
						break;
					case '已下架':
					default:
						response.msg = `当前水果${goodsState}`;
						break;
				}
				return response;
			}
	...
	async payValidate(order_id) {
		...
		orderDetails.forEach((x) => {
			...
			if (goods.stock_num < x.goods_num) {
				this.throw(`水果剩余库存 ${goods.stock_num}`);
			}
