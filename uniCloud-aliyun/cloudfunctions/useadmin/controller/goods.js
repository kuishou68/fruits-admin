...
module.exports = class GoodsController extends Controller {
	...
	validate(req) {
		...
		if (!req.goods) this.throw('水果参数不存在');
		if (!req.detail) this.throw('水果详情参数不存在');

		if (!req.goods.name) this.throw('水果名称必填');
		if (!req.goods.cid) this.throw('请选择水果分类');
		if (req.goods.price <= 0) this.throw('商城价格不能小于0');
		if (!req.goods.sold_out_time) this.throw('请选择下架时间');
		if (!req.goods.img) this.throw('请上传水果主图');
	}
