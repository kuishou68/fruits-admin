<template>
	<div class="container add_edit">
		<el-form :model="form" :rules="rules" ref="ruleForm" label-width="100px">
			...
			<el-form-item label="导航名称" prop="name" ref="name"><el-input v-model="form.name" placeholder="请输入名称"></el-input></el-form-item>
			...
			<el-form-item label="地址">
				<el-select v-model="mpPageUrl" placeholder="请选择" :disabled="form.type == '网页'" class="sele" @change="urlChange">
					<el-option v-for="item in urlOptions" :key="item._id" :label="item.name + ' ' + item.url" :value="item._id"></el-option>
				</el-select>
			</el-form-item>
...
export default {
	...
	methods: {
		async loadData() {
			await this.$db[__goodsCategory].totree({ orderby: 'sort asc', startWith: 'pid == ""' }).then(res => {
				console.log(res);
				if (res.code == 200) {
