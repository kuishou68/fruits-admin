		...
		<div class="container use-table">
			...
			<el-table :height="tblHeight" :data="tableDatas" @selection-change="selectionChange" highlight-current-row>
				...
				<el-table-column property="img" label="图片" align="center">
					<template slot-scope="scope">
						<el-image style="width: 120px;" :src="scope.row.img" :preview-src-list="scope.row.imgs" fit="contain"></el-image>
					</template>
				</el-table-column>
				...
				<el-table-column property="state" label="状态" align="center">
					<template slot-scope="scope">
						<el-tooltip :content="scope.row.state == '启用' ? '点击禁用' : '点击启用'" placement="top" :hide-after="1000" :enterable="false" effect="light">
							<el-switch
	...
	methods: {
		async loadData() {
			await this.$db[__name]
				.whereif(this.req.url, { url: new RegExp(this.req.url) })
				.whereif(this.req.type, { type: this.req.type })
