	...
	<div>
		...
		<div class="container use-table">
			...
			<el-table :height="tblHeight" :data="tableDatas" @selection-change="selectionChange" highlight-current-row>
				...
				<el-table-column label="分类搜索" align="center">
					<template slot-scope="scope">
						<el-tooltip :content="scope.row.is_search == '否' ? '分类搜索' : '取消分类搜索'" placement="top" :hide-after="1000" :enterable="false" effect="light">
							<el-switch
...
export default {
	...
	methods: {
		...
		toAdd() {
			uni.navigateTo({
				url: `/pages/goods/label/goods_tag_add_edit?tab=添加标签`,
				events: {
					refreshData: () => {
						this.loadData();
					}
		...
		toEdit(id) {
			uni.navigateTo({
				url: `/pages/goods/label/goods_tag_add_edit?id=${id}&tab=编辑标签`,
				events: {
					refreshData: () => {
						this.loadData();
					}
		...
		classChange(row) {
			this.$db[__name].update(row._id, { is_search: row.is_search }).then(res => {
