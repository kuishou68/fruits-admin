	...
	<div>
			...
			<el-col :span="8">
				<div class="left_content dflex_c">
					<div class="detail_left center_just">
						<el-image
							class="headimg"
							:src="memberData.member_headimg || '../../../static/user/default.png'"
							fit="contain"
						></el-image>
				...
				<div class="right_content dflex">
					<div class="dflex_vertical_c order">
						...
						<div>
							...
							<v-countup class="order_data" start-value="0" :end-value="memberData.member_coupon_cnt"></v-countup>
						</div>
					</div>
					<div class="dflex_vertical_c order">
						...
						<div>
							...
							<v-countup class="order_data" start-value="0" :end-value="memberData.member_monetary" decimals="2"></v-countup>
						</div>
					</div>
					<div class="dflex_vertical_c order">
						...
						<div>
							...
							<v-countup class="order_data" start-value="0" :end-value="memberData.member_login_cnt"></v-countup>
						</div>
					</div>
					<div class="dflex_vertical_c order">
						...
						<div>
							...
							<v-countup class="order_data" start-value="0" :end-value="memberData.member_amount"></v-countup>
						</div>
					</div>
				</div>
			</el-col>
		...
		<div class="detail pos_r">
			<el-tabs v-model="tab" @tab-click="cutTabs">
				<el-tab-pane :label="`订单 ${totalObj.order_cnt || 0}`" name="order"><order v-if="tab == 'order'"></order></el-tab-pane>
				<el-tab-pane :label="`足迹 ${totalObj.footprint_cnt || 0}`" name="footprint"><footprint v-if="tab == 'footprint'"></footprint></el-tab-pane>
				<el-tab-pane :label="`收藏 ${totalObj.collect_cnt || 0}`" name="collect"><collect v-if="tab == 'collect'"></collect></el-tab-pane>
		...
		async loadData() {
			...
			const totalDatas = await Promise.all([
				this.$db['usemall-order']
					.collection()
					.where({ is_delete: 0, create_uid: this.dataId })
					.count(),
