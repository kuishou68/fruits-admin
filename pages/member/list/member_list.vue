		...
		<div class="container padding_b_0">
			<div class="dflex_wrap">
				<div class="dflex_vertical_c margin_r_40 margin_b_20">
					...
					<el-input v-model="req.member_name" placeholder="请输入" class="search_input" @input="search" clearable></el-input>
				</div>
				...
				<el-table-column label="会员" align="center">
					<template slot-scope="scope">
						<div class="dflex_vertical_c">
							<el-image
								class="headimg_small"
								:src="scope.row.member_headimg || '../../../static/user/default.png'"
								:preview-src-list="scope.row.imgs"
								fit="cover"
							></el-image>
							<div class="margin_l_5">{{ scope.row.member_name || '会员' }}</div>
						</div>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" align="center" fixed="right">
					<template slot-scope="scope">
						<el-tooltip content="详情" placement="top" :hide-after="1000" :enterable="false" effect="light">
							<el-button type="text" icon="iconfont iconxiangqing1" class="bbb" @click="toDetail(scope.row._id)"></el-button>
						</el-tooltip>
					</template>
				</el-table-column>
			</el-table>
	...
	methods: {
		async loadData() {
			...
			await this.$db[__name]
				...
				.whereif(this.req.member_gender || this.req.member_gender === 0, { member_gender: this.req.member_gender })
				...
				.then(res => {
					if (res.code == 200) {
						res.datas.rows.forEach((row, idx) => {
							row.imgs = [row.member_headimg || '../../../static/user/default.png'];
		...
		toAdd() {
			uni.navigateTo({
				url: `/pages/member/list/member_list_add_edit?tab=添加会员`,
				events: {
					refreshData: () => {
						this.loadData();
					}
