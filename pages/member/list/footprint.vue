<template>
	<div>
		...
		<div class="container use-table">
			<el-table ref="eltbl" :height="tblHeight" :data="tableDatas" highlight-current-row>
				...
				<el-table-column property="last_modify_time" label="浏览时间" align="center"></el-table-column>
			</el-table>
			<el-pagination
	...
	methods: {
		async loadData() {
			this.$db[__name]
				...
				.field('visit_cnt, last_modify_time, goods.name, goods.name_pw, goods.img, goods.price, goods.market_price')
				.totable(this.req)
				.then(res => {
					if (res.code == 200) {
