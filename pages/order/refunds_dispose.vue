				...
				<el-table-column label="会员" align="center">
					<template slot-scope="scope">
						<div class="dflex_vertical_c">
							<el-image
								class="headimg_small"
								:src="scope.row.member_headimg || 'https://mall-os-api.use-cloud.com/files/upload/image/20201024/201024115813649391.jpg'"
					...
					<template slot-scope="scope">
						<div v-if="scope.row.state == '处理中'">
							<el-tooltip content="同意退款" placement="top" :hide-after="1000" :enterable="false" effect="light">
								<el-button type="text" icon="el-icon-check" class="bbb" @click="refunded(scope.row._id)"></el-button>
							</el-tooltip>
							<el-tooltip content="拒绝退款" placement="top" :hide-after="1000" :enterable="false" effect="light">
								<el-button type="text" icon="el-icon-close" class="bbb" @click="refuseRefund(scope.row._id)"></el-button>
							</el-tooltip>
						</div>
					</template>
				</el-table-column>
	...
	methods: {
		...
		refunded(id) {
			this.$confirm('此操作将同意退款！', '提示', {
				...
				this.$db[__name].update(id, { remark: '退款成功', state: '已退款' }).then(res => {
		...
		submitForm(formName) {
			this.$refs[formName].validate((valid, obj) => {
				...
				if (valid) {
					this.$db[__name].update(this.refundId, { remark: this.form.refund, state: '拒绝退款' }).then(res => {
						if (res.code == 200) {
